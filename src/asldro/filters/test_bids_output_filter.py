...
def test_bids_output_filter_mock_data_ground_truth():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        d = {
            ...
            "Quantity": "T1map",
            "ComplexImageComponent": "REAL",
            "ImageType": ["ORIGINAL", "PRIMARY", "T1MAP", "NONE",],
            "Manufacturer": "Gold Standard Phantoms",
            "PulseSequenceDetails": f"Digital Reference Object Data generated by ASLDRO, version {asldro_version}",
        }
        ...
        # Check filenames
        assert bids_output_filter.outputs["filename"][0] == os.path.join(
            temp_dir, "sub-001", "ground_truth", "sub-001_prefix_acq-110_T1map.nii.gz",
        )
        assert bids_output_filter.outputs["filename"][1] == os.path.join(
            temp_dir, "sub-001", "ground_truth", "sub-001_prefix_acq-110_T1map.json",
        )
...
def test_bids_output_filter_mock_data_ground_truth_seg_label():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        d = {
            ...
            "ComplexImageComponent": "REAL",
            "ImageType": ["ORIGINAL", "PRIMARY", "DSEG", "NONE",],
            "Manufacturer": "Gold Standard Phantoms",
            "PulseSequenceDetails": f"Digital Reference Object Data generated by ASLDRO, version {asldro_version}",
        }
        ...
        # Check filenames
        assert bids_output_filter.outputs["filename"][0] == os.path.join(
            temp_dir, "sub-001", "ground_truth", "sub-001_prefix_acq-110_dseg.nii.gz",
        )
        assert bids_output_filter.outputs["filename"][1] == os.path.join(
            temp_dir, "sub-001", "ground_truth", "sub-001_prefix_acq-110_dseg.json",
        )
